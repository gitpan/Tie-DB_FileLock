...
my $dbh = new Tie::DB_FileLock::BTREEINFO ;
...
ok(19, $X = tie(%h, 'Tie::DB_FileLock',$Dfile, O_RDWR|O_CREAT, 0640, $DB_BTREE )) ;
...
ok(26, $X = tie(%h,'Tie::DB_FileLock',$Dfile, O_RDWR, 0640, $DB_BTREE)) ;
...
ok(45, $X = tie(%h, 'Tie::DB_FileLock',$Dfile, O_RDWR, 0640, $DB_BTREE ));
...
ok(74, $Y = tie(%h, 'Tie::DB_FileLock',undef, O_RDWR|O_CREAT, 0640, $DB_BTREE ));
...
# Duplicate keys
my $bt = new Tie::DB_FileLock::BTREEINFO ;
$bt->{flags} = R_DUP ;
ok(76, $YY = tie(%hh, 'Tie::DB_FileLock', $Dfile, O_RDWR|O_CREAT, 0640, $bt )) ;
...
$dbh1 = new Tie::DB_FileLock::BTREEINFO ;
{ local $^W = 0 ;
  $dbh1->{compare} = sub { $_[0] <=> $_[1] } ; }
 
$dbh2 = new Tie::DB_FileLock::BTREEINFO ;
$dbh2->{compare} = sub { $_[0] cmp $_[1] } ;
 
$dbh3 = new Tie::DB_FileLock::BTREEINFO ;
$dbh3->{compare} = sub { length $_[0] <=> length $_[1] } ;
 
 
tie(%h, 'Tie::DB_FileLock',$Dfile1, O_RDWR|O_CREAT, 0640, $dbh1 ) ;
tie(%g, 'Tie::DB_FileLock',$Dfile2, O_RDWR|O_CREAT, 0640, $dbh2 ) ;
tie(%k, 'Tie::DB_FileLock',$Dfile3, O_RDWR|O_CREAT, 0640, $dbh3 ) ;
...
ok(89, tie(%h, 'Tie::DB_FileLock', $Dfile1, O_RDWR|O_CREAT, 0640, $DB_BTREE ) );
foreach (1 .. 10)
  { $h{$_} = $_ * 100 }
...

{
    ...
    my $filename = "xyz" ;
    my @x ;
    eval { tie @x, 'Tie::DB_FileLock', $filename, O_RDWR|O_CREAT, 0640, $DB_BTREE ; } ;
    ok(92, $@ =~ /^Tie::DB_FileLock can only tie an associative array to a DB_BTREE database/) ;
    unlink $filename ;
}

{
   ...
   require Exporter ;
   use Tie::DB_FileLock;
   @ISA=qw(Tie::DB_FileLock);
   @EXPORT = @Tie::DB_FileLock::EXPORT ;
...

{
   ...
   ok(103, $db = tie(%h, 'Tie::DB_FileLock', $Dfile, O_RDWR|O_CREAT, 0640, $DB_BTREE ) );
...
{    
    ...
    unlink $Dfile;
    ok(124, $db = tie(%h, 'Tie::DB_FileLock', $Dfile, O_RDWR|O_CREAT, 0640, $DB_BTREE ) );
...

{
   ...
   ok(147, $db = tie(%h, 'Tie::DB_FileLock', $Dfile, O_RDWR|O_CREAT, 0640, $DB_BTREE ) );
...

{
  ...
  my $file = "xyzt" ;
  {
    ...
    unlink "tree" ;
    tie %h, "Tie::DB_FileLock", "tree", O_RDWR|O_CREAT, 0640, $DB_BTREE 
...
EOM
   
  {
    ...
    tie %h, "Tie::DB_FileLock", $filename, O_RDWR|O_CREAT, 0640, $DB_BTREE 
...
EOM

  {
    ...
    $x = tie %h, "Tie::DB_FileLock", $filename, O_RDWR|O_CREAT, 0640, $DB_BTREE 
	or die "Cannot open $filename: $!\n";
